...
public class ItemMenu {
   ...
   */
  @Nullable
  public static MenuItem EMPTY_SLOT_ITEM;
  ...
  static {
    try {
      EMPTY_SLOT_ITEM = new StaticColoredPaneItem(DyeColor.GRAY);
   ...
   */
  public ItemMenu(Plugin plugin, @Nullable final String name, @Nullable final Size size,
                  @Nullable final ItemMenu parent, @Nullable final MenuItem[] items) {
    this.plugin = plugin;
    this.name = name;
    this.size = size;
    this.parent = parent;
  ...
  public ItemMenu setItem(final int position, final MenuItem menuItem) {
    Preconditions.checkArgument(position >= 0 && position < size.getSize(),
                                "Position must be between zero and %s. was ", size.getSize(), position);
  ...
  public ItemMenu update(@NotNull final Player player) {
    if (player.getOpenInventory() != null) {
      ...
      if (inventory.getHolder() instanceof ItemMenuHolder && ((ItemMenuHolder) inventory.getHolder()).getMenu().equals(
        this)) {
  ...
  public ItemMenu open(@NotNull final Player player) {
    ...
    final Inventory inventory = Bukkit.createInventory(
      new ItemMenuHolder(this, Bukkit.createInventory(player, size.getSize())), size.getSize(), name);
    apply(inventory, player);
